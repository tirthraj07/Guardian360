...
class CurrentLocationRepository:
    ...
    def get_location_by_user_id(user_id):
        location = supabase.table("current_location").select("userID, latitude, longitude, timestamp").eq("userID", user_id).execute()
        return location.data[0] if location.data else {}
