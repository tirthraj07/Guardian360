...
async def send_verification_code(request: UserRequest, db: Session = Depends(get_db)):
    email = request.email
...
def verify_code(request: VerifyCodeRequest, db: Session = Depends(get_db)):
    ...
    crud.delete_verification_code(db, email=email)
    crud.create_user(db, first_name=first_name, last_name=last_name, email=email, phone_no=phone_no)
